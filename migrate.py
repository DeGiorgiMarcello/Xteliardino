import pandas as pd
from db import insert_player, insert_match, insert_match_participants, update_players_elo
...
if __name__ == "__main__":
    ...
    for _, match in df.iterrows():
        ...
        date = match["DATE"]
        match_id = insert_match(score_A=match["SCORE_A"], score_B=match["SCORE_B"])
        team_A_elo = sum(players_score[p] for p in [match["GA1"], match["GA2"]])
        team_B_elo = sum(players_score[p] for p in [match["GB1"], match["GB2"]])

        score_team_A, score_team_B = compute_delta(team_A_elo, team_B_elo, match["SCORE_A"], match["SCORE_B"])
        player_score_A = round(score_team_A / 2)
        player_score_B = round(score_team_B / 2)

        insert_match_participants(team_id="A",match_id=match_id, player_id=match["GA1"], score_in_match=player_score_A, is_winner=score_team_A > 0)
        insert_match_participants(team_id="A",match_id=match_id, player_id=match["GA2"], score_in_match=player_score_A, is_winner=score_team_A > 0)
        insert_match_participants(team_id="B",match_id=match_id, player_id=match["GB1"], score_in_match=player_score_B, is_winner=score_team_B > 0)
        insert_match_participants(team_id="B",match_id=match_id, player_id=match["GB2"], score_in_match=player_score_B, is_winner=score_team_B > 0)
